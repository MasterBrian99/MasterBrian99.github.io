...
const card = cva("pouf-card bg-surface rounded-card cushion-card", {
  variants: {
    variant: {
      ...
      flush: "p-0",
      tight:
        "px-(--s4) pt-[calc(var(--s4)-var(--lip)/2)] pb-[calc(var(--s4)+var(--lip)/2)]",
    },
    motion: {
      none: "",
...
export function Card({ children, variant, motion, className }: CardProps) {
  return (
    <div className={cx(card({ variant, motion }), className)}>{children}</div>
  )
}
...
const rowCard = cva(
  [
    "pouf-rowcard block w-full border-none text-left text-inherit [font:inherit]",
  ...
  ],
  {
    variants: {
      ...
      selected: {
        ...
        true: [
          "bg-purple cushion-control text-[var(--on-accent)]",
    ...
    compoundVariants: [
      {
        ...
        selected: false,
        className:
          "hover:cushion-row-hover active:cushion-control-active hover:[transform:translateY(-1px)] active:[transform:translateY(1px)]",
      },
    ],
...
export function RowCard({
  ...
  const classNames = cx(
    rowCard({ interactive: !!onClick, selected: !!selected }),
    className
  )
  ...
  return (
    <div className={classNames} data-selected={selected || undefined}>
      {children}
    </div>
  )
}
